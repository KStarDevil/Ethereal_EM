    ...
    public class Post_Controller : BaseController
    {
        ...
        public dynamic GetPost([FromBody] Newtonsoft.Json.Linq.JObject param)
        {
            ...
            try
            {
                // dynamic dd = param;
                // int id = dd.id;
                dynamic mainQuery1 = _repositoryWrapper.Post_Repository.GetPost();
                dynamic mainQuery2 = _repositoryWrapper.Post_Detail_Repository.GetPostDetail();
                if (mainQuery1 == null && mainQuery2 == null)
                {
                    jsondata = new { data = new { mainQuery = "No Data" } };
                }
                else
                {
                    jsondata = new { data = new { mainQuery1,mainQuery2 } };
                }
            }
            catch (Exception ex)
            {
