...
namespace Ethereal_EM.Repository
{
    public class AdminMenuRepository : RepositoryBase<Adminmenu>, IAdminMenuRepository
    {
            ...
            return (from main in RepositoryContext.Adminmenu
                    ...
                    select new
                    {
                        ...
                        main.ControllerName
                            ...
                            select new
                            {
                                AdminMenuID = detail.MenuID,
                                ParentID = 0,
                                SrNo = 10000,
                                AdminMenuName = "",
                                Icon = "",
                                ControllerName = detail.ControllerName
                            }
                    )
                    .ToList()
                    .Select(q => new
        ...
        public IEnumerable<dynamic> GetAdminTransactionMenu(int adminLevelID)
        {

            var query = (from main in RepositoryContext.Adminmenu
                         where main.SrNo <= 1000 && main.ParentID == 106
                         orderby main.ParentID, main.SrNo
                         select new
                         {
        ...
        public IEnumerable<dynamic> GetAdminTransactionMenuByAdminLevel(int adminLevelID)
        {
            var query = (from ULM in RepositoryContext.Adminlevelmenu
                         join M in RepositoryContext.Adminmenu on ULM.AdminMenuID equals M.AdminMenuID
                         where ULM.AdminLevelID == adminLevelID && M.SrNo <= 1000 && M.ParentID == 106
                         orderby M.ParentID, M.SrNo
                         select new
                         {
                             M.AdminMenuID,
